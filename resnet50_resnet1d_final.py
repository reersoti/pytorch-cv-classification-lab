...
def main():
    ...
    output_dir = "./features_resnet50"
    os.makedirs(output_dir, exist_ok=True)
    
    # Load features
    train_path, test_path = os.path.join(output_dir, "train_r50.pt"), os.path.join(output_dir, "test_r50.pt")
    ...
    model = ResNet1D(input_dim=2048, num_classes=num_classes).to(device)
    ...
    for ep in range(1, num_epochs + 1):
        ...
        acc = corr / len(v_db)
        if acc > best_acc:
            best_acc = acc; torch.save(model.state_dict(), "resnet50_resnet1d.pth")
        print(f"Epoch {ep}/{num_epochs} | Val Acc: {acc:.4f} (Best: {best_acc:.4f})")
    
    # Inference
    print("\n--- Generating Submission ---")
    model.load_state_dict(torch.load("resnet50_resnet1d.pth", weights_only=True))
    model.eval()
    with torch.no_grad():
        all_preds = []
    ...
    pd.DataFrame({"id": test_data["labels"].tolist(), "label": all_preds}).sort_values("id").to_csv("submission_r50_resnet1d.csv", index=False)
    print("Done!")
