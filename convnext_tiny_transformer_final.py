...
def main():
    ...
    output_dir="./features_cnext";os.makedirs(output_dir,exist_ok=True)
    train_path,test_path=os.path.join(output_dir,"train_cnext.pt"),os.path.join(output_dir,"test_cnext.pt")
    if os.path.exists(train_path)and os.path.exists(test_path):print("\n--- Loading convnext_tiny Features ---");train_data,test_data=torch.load(train_path,weights_only=True),torch.load(test_path,weights_only=True)
    else:print("\n--- Run feature extraction first ---");return
    print("\n--- Training convnext_tiny + TransformerHead ---")
    ...
    model=TransformerHead(input_dim=768,num_classes=num_classes).to(device)
    ...
    for ep in range(1,num_epochs+1):
        ...
        acc=corr/len(v_db)
        if acc>best_acc:best_acc=acc;torch.save(model.state_dict(),"convnext_tiny_transformer.pth")
        print(f"Epoch {ep}/{num_epochs} | Val Acc: {acc:.4f} (Best: {best_acc:.4f})")
    print("\n--- Generating Submission ---");model.load_state_dict(torch.load("convnext_tiny_transformer.pth",weights_only=True));model.eval();all_preds=[]
    with torch.no_grad():
    ...
    pd.DataFrame({"id":test_data["labels"].tolist(),"label":all_preds}).sort_values("id").to_csv("submission_cnext_trans.csv",index=False);print("Done!")
