...
def main():
    ...
    output_dir="./features_r34";os.makedirs(output_dir,exist_ok=True)
    train_path,test_path=os.path.join(output_dir,"train_r34.pt"),os.path.join(output_dir,"test_r34.pt")
    if os.path.exists(train_path)and os.path.exists(test_path):print("\n--- Loading resnet34 Features ---");train_data,test_data=torch.load(train_path,weights_only=True),torch.load(test_path,weights_only=True)
    else:print("\n--- Run feature extraction first ---");return
    print("\n--- Training resnet34 + TransformerHead ---")
    ...
    model=TransformerHead(input_dim=512,num_classes=num_classes).to(device)
    ...
    for ep in range(1,num_epochs+1):
        ...
        acc=corr/len(v_db)
        if acc>best_acc:best_acc=acc;torch.save(model.state_dict(),"resnet34_transformer.pth")
        print(f"Epoch {ep}/{num_epochs} | Val Acc: {acc:.4f} (Best: {best_acc:.4f})")
    print("\n--- Generating Submission ---");model.load_state_dict(torch.load("resnet34_transformer.pth",weights_only=True));model.eval();all_preds=[]
    with torch.no_grad():
    ...
    pd.DataFrame({"id":test_data["labels"].tolist(),"label":all_preds}).sort_values("id").to_csv("submission_r34_trans.csv",index=False);print("Done!")
