...
class TransformerHead(nn.Module):
    def __init__(self, input_dim=2048, d_model=64, nhead=8, num_layers=4, num_classes=46):
        super().__init__()
...
def main():
    ...
    output_dir = "./features_resnet50" # Новый кэш для ResNet50
    os.makedirs(output_dir, exist_ok=True)
    num_classes = 46

    # --- PHASE 1: Data Check ---
    train_path = os.path.join(output_dir, "train_r50.pt")
    test_path = os.path.join(output_dir, "test_r50.pt")

    if os.path.exists(train_path) and os.path.exists(test_path):
        print("\n--- PHASE 1: Loading existing ResNet50 Features ---")
        train_data = torch.load(train_path, weights_only=True)
        test_data = torch.load(test_path, weights_only=True)
    else:
        print("\n--- PHASE 1: Extracting ResNet50 Features ---")
        resnet = models.resnet50(weights=models.ResNet50_Weights.IMAGENET1K_V1).to(device)
        resnet.eval()
        # Извлекаем признаки до глобального пулинга
        extractor = nn.Sequential(*list(resnet.children())[:-1], nn.Flatten()).to(device)
        ...
        def run_ext(csv, img_dir, name, is_test):
            ...
            torch.save(res, os.path.join(output_dir, f"{name}_r50.pt" if not is_test else f"{name}_r50.pt"))
            return res
    ...
    # --- PHASE 2: TRAINING ---
    print("\n--- PHASE 2: Training TransformerHead (Backbone: ResNet50) ---")
    ...
    model = TransformerHead(input_dim=2048, num_classes=num_classes).to(device)
    ...
    for ep in range(1, num_epochs + 1):
        ...
        acc = corr / len(v_db)
        if acc > best_acc:
            best_acc = acc; torch.save(model.state_dict(), "best_model_r50_trans.pth")
        print(f"Epoch {ep}/{num_epochs} | Val Acc: {acc:.4f} (Best: {best_acc:.4f})")

    # --- PHASE 3: INFERENCE ---
    print("\n--- PHASE 3: Generating Submission ---")
    model.load_state_dict(torch.load("best_model_r50_trans.pth", weights_only=True))
    model.eval()
    with torch.no_grad():
    ...
    sub.to_csv("submission_resnet_transformer.csv", index=False)
    print("Done! Result saved in submission_resnet_transformer.csv")
