...
def main():
    ...
    print("\n--- Training mobilenetv3_small + ResNet1D ---")
    ...
    model=ResNet1D(input_dim=576,num_classes=num_classes).to(device)
    ...
    for ep in range(1,num_epochs+1):
        ...
        acc=corr/len(v_db)
        if acc>best_acc:best_acc=acc;torch.save(model.state_dict(),"mobilenetv3_small_resnet1d.pth")
        print(f"Epoch {ep}/{num_epochs} | Val Acc: {acc:.4f} (Best: {best_acc:.4f})")
    print("\n--- Generating Submission ---");model.load_state_dict(torch.load("mobilenetv3_small_resnet1d.pth",weights_only=True));model.eval();all_preds=[]
    with torch.no_grad():
    ...
    pd.DataFrame({"id":test_data["labels"].tolist(),"label":all_preds}).sort_values("id").to_csv("submission_mobv3_resnet1d.csv",index=False);print("Done!")
