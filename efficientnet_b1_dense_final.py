...
def main():
    ...
    output_dir = "./features_effb1" # Используем кэш B1
    ...
    # EfficientNet-B1 features dim = 1280
    model = DenseHead(input_dim=1280, num_classes=num_classes).to(device)
    ...
    for ep in range(1, num_epochs + 1):
        ...
        acc = corr / len(v_db)
        if acc > best_acc:
            best_acc = acc; torch.save(model.state_dict(), "efficientnet_b1_dense.pth")
        print(f"Epoch {ep}/{num_epochs} | Val Acc: {acc:.4f} (Best: {best_acc:.4f})")
    
    # Inference
    print("\n--- Generating Submission ---")
    model.load_state_dict(torch.load("efficientnet_b1_dense.pth", weights_only=True))
    model.eval()
    all_preds = []
    with torch.no_grad():
    ...
    # Используем одинарные скобки для словаря DataFrame
    pd.DataFrame({"id": test_data["labels"].tolist(), "label": all_preds}).sort_values("id").to_csv("submission_effb1_dense.csv", index=False)
    print("Done!")
