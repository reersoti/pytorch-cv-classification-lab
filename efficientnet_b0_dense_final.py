...
def main():
    ...
    # Train
    print("\n--- Training EfficientNet-B0 + DenseHead ---")
    ...
    for ep in range(1, num_epochs + 1):
        ...
        acc = corr / len(v_db)
        if acc > best_acc:
            best_acc = acc; torch.save(model.state_dict(), "efficientnet_b0_dense.pth")
        print(f"Epoch {ep}/{num_epochs} | Val Acc: {acc:.4f} (Best: {best_acc:.4f})")
    
    # Inference
    print("\n--- Generating Submission ---")
    model.load_state_dict(torch.load("efficientnet_b0_dense.pth", weights_only=True))
    model.eval()
    with torch.no_grad():
        all_preds = []
    ...
    pd.DataFrame({"id": test_data["labels"].tolist(), "label": all_preds}).sort_values("id").to_csv("submission_effnet_b0_dense.csv", index=False)
    print("Done!")
