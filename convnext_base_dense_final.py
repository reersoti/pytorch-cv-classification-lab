...
def main():
    ...
    output_dir = "./features_cnext_base"
    
    # Load features
    train_path, test_path = os.path.join(output_dir, "train_cnext_base.pt"), os.path.join(output_dir, "test_cnext_base.pt")
    
    if not os.path.exists(train_path):
        print("Error: Features not found! Run extract_all_features.py first.")
        return

    print("\n--- Loading ConvNeXt-Base Features ---")
    train_data = torch.load(train_path, weights_only=True)
    test_data = torch.load(test_path, weights_only=True)
    
    # Train
    print("\n--- Training ConvNeXt-Base + DenseHead ---")
    ...
    # ConvNeXt-Base features dim = 1024
    model = DenseHead(input_dim=1024, num_classes=num_classes).to(device)
    # Используем чуть меньший LR и больший weight_decay для стабильности
    ...
    for ep in range(1, num_epochs + 1):
        ...
        acc = corr / len(v_db)
        if acc > best_acc:
            best_acc = acc; torch.save(model.state_dict(), "convnext_base_dense.pth")
        print(f"Epoch {ep}/{num_epochs} | Val Acc: {acc:.4f} (Best: {best_acc:.4f})")
    
    # Inference
    print("\n--- Generating Submission ---")
    model.load_state_dict(torch.load("convnext_base_dense.pth", weights_only=True))
    model.eval()
    all_preds = []
    with torch.no_grad():
    ...
    pd.DataFrame({"id": test_data["labels"].tolist(), "label": all_preds}).sort_values("id").to_csv("submission_cnext_base_dense.csv", index=False)
    print("Done! Result saved to submission_cnext_base_dense.csv")
